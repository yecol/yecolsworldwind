 ...
 * @version $Id: ScreenAnnotation.java 12733 2009-10-21 13:12:05Z patrickmurris $
...
public class ScreenAnnotation extends AbstractAnnotation
{
    ...
    protected Rectangle computeBounds(DrawContext dc)
    {
        ...
        double x = this.screenPoint.x - width / 2 + offsetX;
        double y = this.screenPoint.y + offsetY; // use OGL coordinate system

        Rectangle frameRect = new Rectangle((int)x, (int)y, (int)width, (int)height);

        // Include reference point in bounds
        return this.computeBoundingRectangle(frameRect, this.screenPoint.x,  this.screenPoint.y);
    }
    ...
    protected void doRenderNow(DrawContext dc)
    {
        ...
        this.drawTopLevelAnnotation(dc, this.screenPoint.x, this.screenPoint.y, size.width, size.height, 1, 1, null);
    }
