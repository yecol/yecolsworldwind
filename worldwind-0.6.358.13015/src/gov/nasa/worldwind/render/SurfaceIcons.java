 ...
 * @version $Id: SurfaceIcons.java 12829 2009-11-26 15:35:39Z patrickmurris $
 */
public class SurfaceIcons extends SurfaceIcon
{
    ...
    public Iterable<? extends Sector> getSectors(DrawContext dc, double texelSizeRadians)
    {
        ...
        if (this.isMaintainAppearance())
            return Arrays.asList(this.computeSector(dc, texelSizeRadians));
        ...
        Iterable<? extends Sector> sectors = Arrays.asList(this.computeSector(dc, texelSizeRadians));
    ...
    protected Sector computeSector(DrawContext dc, double texelSizeRadians)
    {
        ...
        sector = new Sector(
            sector.getMinLatitude().subtractRadians(padLatRadians),
            sector.getMaxLatitude().addRadians(padLatRadians),
            sector.getMinLongitude().subtractRadians(padLonRadians),
            sector.getMaxLongitude().addRadians(padLatRadians)
        );
    ...
    protected double computeLocationDistanceDegreesSquared(Sector drawSector, LatLon location)
    {
        ...
        double dLon = location.getLongitude().degrees - drawSector.getCentroid().getLongitude().degrees;
