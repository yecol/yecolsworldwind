 ...
 * image's lower right corner is aligned with a point <code>(imageWidth, imageHeight)</code> pixels right and down from
 * the annotation's upper left corner. Thus the background image coordinate system has its origin at the annotation's
 * upper left corner, has the +X axis pointing to the right, and has the +Y axis pointing down. Units are in image
 * pixels, where one image pixel corresponds to one screen pixel. The background image may be translated or scaled by
 * setting the attributes {@link gov.nasa.worldwind.render.AnnotationAttributes#setImageOffset(java.awt.Point)} and
 * {@link gov.nasa.worldwind.render.AnnotationAttributes#setImageScale(double)}, respectively. The offset attribute
 * defines an offset right and down in background image coordinates. The scale atrribute is unitless, and defines the
 ...
 * conversion from a non-power-of-two image to a power-of-two image depends on the image's indended use. However, the
 * following two step solution works for most applications:
 * <ol>
 * <li>Create a transparent power-of-two image larger than the original image. The utility method
 * {@link gov.nasa.worldwind.util.WWMath#powerOfTwoCeiling(int)} is useful for computing power-of-two dimensions:
 * <code>
 * <br/><br/>
 * int newWidth = WWMath.powerOfTwoCeiling(originalWidth);<br/>
 * int newHeight = WWMath.powerOfTwoCeiling(originalHeight);
 * <br/><br/>
 * </code>
 * </li>
 * <li>Copy the original image contents into the empty power-of-two image. Any pixels not covered by the original image
 * are left completely transparent:
 * <code>
 * <br/><br/>
 * BufferedImage newImage = new BufferedImage(newWidth, newHeight, BufferedImage.TYPE_INT_ARGB);<br/>
 * Graphics2D g2d = newImage.createGraphics();<br/>
 * try<br/>
 * {<br/>
 * &nbsp;&nbsp;g2d.drawImage(originalImage, 0, 0, null);<br/>
 * }<br/>
 * finally<br/>
 * {<br/>
 * &nbsp;&nbsp;g2d.dispose();<br/>
 ...
 * </code></li>
 * </ol>
 * When used as an Annotation background image, the new power-of-two image appears identical to its original
 * non-power-of-two counterpart, except that the new image is now compatible with most graphics hardware.
 *
...
public abstract class AbstractAnnotation extends AVListImpl implements Annotation
{
    ...
    protected void drawBackgroundImage(DrawContext dc, int width, int height, double opacity, Position pickPosition)
    {
        ...
        WWTexture texture = this.getAttributes().getBackgroundTexture();
        if (texture == null)
            return;
