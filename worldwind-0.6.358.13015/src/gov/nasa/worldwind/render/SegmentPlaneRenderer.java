...
/**
 * @author dcollins
 * @version $Id: SegmentPlaneRenderer.java 12584 2009-09-14 19:25:59Z dcollins $
 */
public class SegmentPlaneRenderer
{
    ...
    protected void begin(DrawContext dc, OGLStackHandler ogsh)
    {
        ...
        OGLStateSupport oglSupport = new OGLStateSupport();
        if (!dc.isPickingMode())
        {
            oglSupport.setEnableBlending(true);
            oglSupport.setEnableLighting(true);
            oglSupport.setColorMode(OGLStateSupport.COLOR_NO_PREMULTIPLIED_ALPHA);
            oglSupport.setLightType(OGLStateSupport.LIGHT_DIRECTIONAL_FROM_VIEWER_POSITION);
            oglSupport.setLightPosition(new Vec4(1.0, 0.5, 1.0));
        }

        int attribMask =
            oglSupport.getAttributeBits()
        ...
        java.awt.Point pickPoint, Layer layer)
    {
        ...
        try
        {
            if ((mask & SegmentPlane.LEFT) != 0)
            {
                ...
                modelview = modelview.multiply(globe.computeModelCoordinateOriginTransform(
        ...
        ControlPointInfo controlPointInfo)
    {
        ...
        Matrix transformToPosition = globe.computeModelCoordinateOriginTransform(controlPointInfo.position);
    ...
    protected static class OrderedText implements OrderedRenderable
    {
        ...
        {
            return dc.getScreenPoint(this.position).add3(attributes.getOffset());
        }
