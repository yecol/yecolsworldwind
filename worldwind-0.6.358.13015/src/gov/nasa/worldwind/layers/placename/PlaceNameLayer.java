...
/**
 * @author Paul Collins
 * @version $Id: PlaceNameLayer.java 13007 2010-01-14 01:57:18Z tgaskins $
 */
...
//        return placeNameService.getExtent(dc).intersects(dc.getView().getFrustumInModelCoordinates());
    ...
    protected static class DownloadPostProcessor implements RetrievalPostProcessor
    {
        ...
        public java.nio.ByteBuffer run(Retriever retriever)
        {
            ...
            try
            {
                ...
                final java.io.File outFile = this.fileStore.newFile(this.tile.getFileCachePath());
                if (outFile == null)
                    return null;
    ...
    {
        PlaceNameLayerBulkDownloader thread = new PlaceNameLayerBulkDownloader(this, sector, resolution, fileStore,
            listener);
        thread.setDaemon(true);
        thread.start();
        return thread;
