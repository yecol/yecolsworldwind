 ...
 *
 * @author dcollins
 * @version $Id: CompoundVecBuffer.java 12787 2009-11-13 18:00:52Z patrickmurris $
 */
public class CompoundVecBuffer extends CompoundBuffer<VecBuffer, VecBuffer>
{
    ...
    /**
     * Creates a CompoundVecBuffer from a subset of another CompoundVecBuffer. The new buffer shares
     * the source buffer backing VecBuffer.
     *
     * @param buffer the source CompoundVecBuffer.
     * @param subBufferOffset the index of the first sub-buffer to include in the subset.
     * @param length the number of sub-buffers to include in the subset.
     * @return a CompoundVecBuffer from a subset of the given CompoundVecBuffer.
     */
    public static CompoundVecBuffer fromSubset(CompoundVecBuffer buffer, int subBufferOffset, int length)
    {
        IntBuffer offsetBuffer = BufferUtil.newIntBuffer(length);
        IntBuffer lengthBuffer = BufferUtil.newIntBuffer(length);
        // Copy sub buffer start positions and length from the source buffer
        for (int i = 0; i < length; i++)
        {
            offsetBuffer.put(buffer.getSubPositionBuffer().get(subBufferOffset + i));
            lengthBuffer.put(buffer.getSubLengthBuffer().get(subBufferOffset + i));
        }
        offsetBuffer.rewind();
        lengthBuffer.rewind();

        BufferFactory bufferFactory = buffer.getBufferFactory();
        return new CompoundVecBuffer(buffer.getBackingBuffer(), offsetBuffer, lengthBuffer, length, bufferFactory);
    }
