 ...
 */
public class Road extends AVListImpl implements Renderable, Movable, Restorable
{
    ...
    public Road(Iterable<? extends Position> positions)
    {
    ...
    public Road(LatLon ll1,LatLon ll2){
    ...
    public Road(Iterable<? extends LatLon> positions, double elevation)
    {
...
//    }
    ...
    protected Extent computeExtent(DrawContext dc)
    {
        ...
        return dc.getGlobe().computeBoundingCylinder(ve, sector, minAndMaxElevations[0], minAndMaxElevations[1]);
    }
...
//        }
    ...
    public void moveTo(Position position)
    {
        ...
        Globe globe = infos.next().globe;

        Vec4 origRef = this.referenceCenterPoint;
        Vec4 newRef = globe.computePointFromPosition(position);
        Angle distance =
        ...
        for (int i = 0; i < this.positions.size(); i++)
        {
            ...
            Vec4 p = globe.computePointFromPosition(pos);
            p = p.transformBy3(q);
            pos = globe.computePositionFromPoint(p);
