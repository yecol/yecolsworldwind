...
/**
 * @author tag
 * @version $Id: DraggingShapes.java 12728 2009-10-19 21:36:25Z dcollins $
 */
public class DraggingShapes extends ApplicationTemplate
{
    private static class AppFrame extends ApplicationTemplate.AppFrame
    {
        ...
        public AppFrame()
        {
            this.shapeLayer = this.buildShapesLayer();
            this.dragger = new BasicDragger(getWwd());

            insertBeforeCompass(this.getWwd(), this.shapeLayer);
        ...
        protected void saveObjects()
        {
            ...
            for (Renderable r : this.shapeLayer.getRenderables())
            {
                ...
                String xmlString = ((Restorable) r).getRestorableState();
                if (xmlString == null)
                    continue;

                File file = new File(dir, r.getClass().getName() + "-" + f.format(counter) + ".xml");
        ...
        protected void loadObjects()
        {
            ...
            FilenameFilter filter = new FilenameFilter()
            {
                ...
                {
                    return name.startsWith("gov.nasa.worldwind.") && name.endsWith(".xml");
                }
            };
            ...
            for (File file : dir.listFiles(filter))
            {
                ...
                try
                {
                    ...
                    Restorable restorable = (Restorable) c.newInstance();
                    restorable.restoreState(xmlString);

                    if (restorable instanceof Renderable)
                    {
                        renderableList.add((Renderable) restorable);
                    }
                }
                catch (Exception e)
                {
            ...
            this.shapeLayer.addRenderables(renderableList);
        }
        ...
        protected RenderableLayer buildShapesLayer()
        {
            ...
            this.setupFilledShape(shape, layer, Color.CYAN);
            ...
            positions.add(new LatLon(Angle.fromDegrees(originLat + 2.5), Angle.fromDegrees(originLon + 5.0)));
            positions.add(new LatLon(Angle.fromDegrees(originLat + 5.0), Angle.fromDegrees(originLon + 2.5)));
            shape = new SurfacePolygon(positions);
            BufferedImage image = PatternFactory.createPattern(PatternFactory.PATTERN_CIRCLES, Color.WHITE);
            this.setupTexturedShape(shape, layer, Color.BLUE, image);     // TODO: uncomment to test texture
            //this.setupFilledShape(shape, layer, Color.BLUE);                // TODO: comment out when using texture
            ...
            positions.add(new LatLon(Angle.fromDegrees(-10), Angle.fromDegrees(-170)));
            shape = new SurfacePolyline(positions);
            this.setupLine(shape, layer, Color.RED, false);

            // Test +180/-180 lon span Polygon
            positions = new ArrayList<LatLon>();
            ...
            positions.add(new LatLon(Angle.fromDegrees(20), Angle.fromDegrees(-170)));
            shape = new SurfacePolygon(positions);
            this.setupFilledShape(shape, layer, Color.RED);

            // Polylines around San Juan County, WA
            Iterable<? extends LatLon> iterable = makeLatLon(SAN_JUAN_COUNTY_1);
            shape = new SurfacePolyline(iterable);
            this.setupLine(shape, layer, Color.YELLOW, true);

            iterable = makeLatLon(SAN_JUAN_COUNTY_2);
            shape = new SurfacePolyline(iterable);
            this.setupLine(shape, layer, Color.YELLOW, true);

            iterable = makeLatLon(SAN_JUAN_COUNTY_3);
            shape = new SurfacePolyline(iterable);
            this.setupLine(shape, layer, Color.YELLOW, true);

            // Polyline around Snohomish County, WA
            iterable = makeLatLon(SNOHOMISH_COUNTY);
            shape = new SurfacePolyline(iterable);
            this.setupLine(shape, layer, Color.YELLOW, true);
        ...
        protected void setupFilledShape(AbstractSurfaceShape shape, SurfaceShapeLayer layer, Color color)
        {
            ShapeAttributes attributes = new BasicShapeAttributes();
            attributes.setDrawInterior(true);
            attributes.setDrawOutline(true);
            attributes.setInteriorMaterial(new Material(color));
            attributes.setOutlineMaterial(new Material(this.makeBrighter(color)));
        ...
        protected void setupTexturedShape(AbstractSurfaceShape shape, SurfaceShapeLayer layer, Color color, Object imageSource)
        {
            ShapeAttributes attributes = new BasicShapeAttributes();
            attributes.setDrawInterior(true);
            attributes.setDrawOutline(true);
            ...
            attributes.setInteriorImageScale(2);
            shape.setAttributes(attributes);
            
            layer.addRenderable(shape);
        }

        protected void setupLine(AbstractSurfaceShape shape, SurfaceShapeLayer layer, Color color, boolean closed)
        {
            ShapeAttributes attributes = new BasicShapeAttributes();
            ...
            attributes.setOutlineWidth(3);
            attributes.setOutlineStippleFactor(2);
            shape.setAttributes(attributes);

            ((SurfacePolyline) shape).setClosed(closed);
    ...
    private static Iterable<? extends LatLon> makeLatLon(double[] src, int offset, int length)
    {
    ...
    private static Iterable<? extends LatLon> makeLatLon(double[] src)
    {
