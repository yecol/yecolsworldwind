...
/**
 * @author dcollins
 * @version $Id: AbstractSurfaceShape.java 12911 2009-12-12 12:54:55Z tgaskins $
 */
public abstract class AbstractSurfaceShape extends AbstractSurfaceObject implements SurfaceShape, Movable
{
    ...
    public Iterable<? extends Sector> getSectors(DrawContext dc, double texelSizeRadians)
    {
        ...
        Iterable<? extends Sector> sectors = this.doGetSectors(dc, texelSizeRadians);
    ...
    {
        Iterable<? extends LatLon> locations = this.getLocations(dc.getGlobe());
    ...
    {
        SurfaceShapeSupport shapeSupport = getSurfaceShapeSupport();
        shapeSupport.beginRendering(dc, sector, x, y, width, height);
    }

    protected void endRenderToRegion(DrawContext dc)
    {
        SurfaceShapeSupport shapeSupport = getSurfaceShapeSupport();
        shapeSupport.endRendering(dc);
    }
    ...
    protected void doRenderBoundingSectors(DrawContext dc, Sector sector, int x, int y, int width, int height)
    {
        ...
        SurfaceShapeSupport shapeSupport = getSurfaceShapeSupport();
        shapeSupport.applyMaterialState(dc, Material.GREEN, 1);
        gl.glLineWidth(1f);

        for (Sector s : this.getSectors(dc, texelSizeRadians))
        {
            shapeSupport.drawLocations(dc, GL.GL_LINE_LOOP, s, 4);
        }
    }
