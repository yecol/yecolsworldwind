 ...
 * base altitude, both in meters</li> <li>Color : the grid point's RGB color components</li> <li>Opacity : the grid
 * point's Alpha color component</li> </ul> Callers specify the attributes at each grid point by invoking {@link
 * #setValues(Iterable)} with an {@link Iterable} of {@link GridPointAttributes}. Grid points are assigned attributes
 * from this iterable staring at the upper left hand corner, and proceeding in row-first order across the grid. The
 * iterable should contain at least <code>width * height</code> values, where width and height are the AnalyticSurface's
 * grid dimensions. If the caller does not specify any GridPointAttributes, or the caller specified iterable contains
 * too few values, the unassigned grid points are given default attributes: the default scalar value is 0, the default
 * color is {@link java.awt.Color#BLACK}, and the default opacity is 1 (totally opaque).
 *
 * @author dcollins
 * @version $Id: AnalyticSurface.java 12811 2009-11-19 19:35:00Z dcollins $
 */
public class AnalyticSurface implements Renderable, PreRenderable
{
    ...
    public interface GridPointAttributes
    {
         ...
         * Color as the RGB components of a grid point's RGBA color, ignoring the alpha channel.
         *
        ...
        /**
         * Returns the opacity associated with a grid point. By default, AnalyticSurface interprets this opacity as the
         * Alpha component of a grid point's RGBA color, overriding the alpha component in {@link #getColor()} if one
         * exists.
         *
         * @return the grid point's Alpha color component.
         */
        double getOpacity();
    }
    ...
    protected static final GridPointAttributes DEFAULT_GRID_POINT_ATTRIBUTES = createGridPointAttributes(
        DEFAULT_VALUE, DEFAULT_COLOR, DEFAULT_OPACITY);
     ...
     * default scalar value is 0, the default color is {@link java.awt.Color#BLACK}, and the default opacity is 1
     * (totally opaque).
     *
    ...
    /**
     * Returns a new instance of {@link GridPointAttributes} with the specified value, color, and opacity.
     *
     * @param value   the new GridPointAttributes' value.
     * @param color   the new GridPointAttributes' color.
     * @param opacity the new GridPointAttributes' opacity.
     *
     * @return a new GridPointAttributes defined by the specified value, color, and opaicty.
     */
    ...
    /**
     * Returns a new instance of {@link GridPointAttributes} with Color and opacity computed by the specified value and
     * value range. The color hue is computed by mapping value's relative position in the range <code>[minValue,
     * maxValue]</code> to the range of color hues <code>[minHue, maxHue]</code>. The opacity is computed by mapping the
     * values's relative position in the range <code>[minValue, minValue + (maxValue - minValue) / 10]</code> to the
     * opacity range <code>[0, 1]</code>. This has the effect of interpolating hue and opacity based on the grid point
     * value.
     *
     * @param value    the new GridPointAttributes' value.
        ...
        double minValue, double maxValue, double minHue, double maxHue)
    {
        ...
        return createGridPointAttributes(value, color, opacity);
    }

    /**
     * Returns a new iterable populated with the default {@link GridPointAttributes}. The default GridPointAttributes
     * have a value of 0, the color {@link java.awt.Color#BLACK}, and an opacity of 0.
     *
    ...
    protected void drawShapeOutline(DrawContext dc)
    {
        ...
        if (!dc.isPickingMode())
        {
            ...
            OGLUtil.applyColor(gl, this.surfaceAttributes.getOutlineMaterial().getDiffuse(),
                this.surfaceAttributes.getOutlineOpacity(), false);
        }
    ...
    protected void beginShapeRendering(DrawContext dc)
    {
        ...
        else
        {
            ...
            stateSupport.setEnableLighting(true);
            stateSupport.setColorMode(OGLStateSupport.COLOR_NO_PREMULTIPLIED_ALPHA);
            stateSupport.setLightType(OGLStateSupport.LIGHT_DIRECTIONAL_FROM_VIEWER_POSITION);
            stateSupport.setLightPosition(new Vec4(1.0, 0.5, 1.0));
        }
        ...
        Vec4 referencePoint, RenderInfo outRenderInfo)
    {
        ...
        if (attr != null)
            opacity = attr.getOpacity() * this.surfaceAttributes.getInteriorOpacity();
