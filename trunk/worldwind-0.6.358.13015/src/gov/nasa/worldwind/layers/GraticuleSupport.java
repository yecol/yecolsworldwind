...
public class GraticuleSupport
{
    ...
    private TiledSurfaceObjectRenderer surfaceRenderer = new TiledSurfaceObjectRenderer();
    ...
    public void preRender(DrawContext dc, double opacity)
    {
        ...
        ArrayList<SurfaceObject> surfaceObjects = new ArrayList<SurfaceObject>();
        ...
        for (Pair pair : this.surfaceRenderables)
        {
            ...
            if (renderable != null && renderable instanceof SurfacePolyline)
            {
                if (renderingParams == null || renderingParams.isDrawLines())
                {
                    ...
                    surfaceObjects.add((SurfacePolyline) renderable);
                }
            }
        }

        this.surfaceRenderer.setSurfaceObjects(surfaceObjects);
    ...
    public void render(DrawContext dc, double opacity)
    {
        ...
        if (this.surfaceRenderables.size() > 0)
            this.surfaceRenderer.render(dc);
    ...
    private void applyRenderingParams(String key, AVList params, SurfacePolyline polyline, double opacity)
    {
        ...
        {
            ShapeAttributes newAttrs = this.getLineShapeAttributes(key, params, opacity);
            if (!polyline.getAttributes().equals(newAttrs))
                polyline.setAttributes(newAttrs);
        }
    }
